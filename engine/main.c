/* command line interpretation, image loading etc. for Gforth
  ...
  Copyright (C) 1995,1996,1997,1998,2000,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025 Free Software Foundation, Inc.
			  ...
			  Char *bitstring, Char *targets)
{
  ...
  debugp(stderr, "compile code range %p:%llx\n", image, (unsigned long long)(UCell)size);
  jit_write_enable();
  for(i=k=0; k<steps; k++) {
    Char bitmask;
		     ...
		     UCell sizes[], Cell bases[])
{
  ...
  for (i=0; i<=PRIMSECTION; i++) {
    ...
    debugp(stderr, "relocate section %i, %p:%llx\n", i, (void *)base, (unsigned long long)size);
...
Address alloc_mmap(Cell size)
{
  ...
  if (MAP_32BIT && map_32bit) {
    debugp(stderr,"try mmap(%p, $%llx, %x, %x, %i, %i); ", (void*)0, (unsigned long long)(UCell)size, prot_exec|PROT_READ|PROT_WRITE, MAP_ANON|MAP_PRIVATE|map_noreserve|map_extras|MAP_32BIT, dev_zero, 0);
    r=
#ifdef PROT_RWX_FAIL
  ...
  if (r==MAP_FAILED) {
    debugp(stderr,"try mmap(%p, $%llx, %x, %x, %i, %i); ", (void*)0, (unsigned long long)(UCell)size, prot_exec|PROT_READ|PROT_WRITE, MAP_ANON|MAP_PRIVATE|map_noreserve|map_extras, dev_zero, 0);
    r=
#ifdef PROT_RWX_FAIL
      prot_exec ? ({ errno=EPERM; MAP_FAILED; }) :
#endif
  ...
  if (r==MAP_FAILED) {
	...
	mmap(0, size, prot_exec|PROT_READ|PROT_WRITE, MAP_ANON|MAP_PRIVATE|map_noreserve|map_extras, dev_zero, 0);
      ...
      debugp(stderr,"try mmap(%p, $%llx, %x, %x, %i, %i); ", (void*)0, (unsigned long long)(UCell)size, PROT_READ|PROT_WRITE, MAP_ANON|MAP_PRIVATE|map_noreserve|map_extras, dev_zero, 0);
...
static void *dict_alloc_read(FILE *file, Cell imagesize, Cell dictsize, Cell offset)
{
  ...
  if (offset==0) {
    ...
    if (image != (void *)MAP_FAILED) {
      void *image1;
      debugp(stderr, "mmap($%lx) succeeds, address=%p\n", (long)dictsize, image);
      debugp(stderr,"try mmap(%p, $%llx, RWX, MAP_FIXED|MAP_FILE, imagefile, 0); ", image, (unsigned long long)(UCell)imagesize);
      image1=
#ifdef PROT_RWX_FAIL
	PROT_EXEC ? ({ errno=EPERM; MAP_FAILED; }) :
#endif
...
#if defined(__ANDROID__) || defined(_AIX) || defined(_WIN32)
      if (image1 == (void *)MAP_FAILED)
	goto read_image;
#endif
      if (image1 == (void *)MAP_FAILED) {
        debugp(stderr,"try mmap(%p, $%llx, RW, MAP_FIXED|MAP_FILE, imagefile, 0); ", image, (unsigned long long)(UCell)imagesize);
  ...
  read_image: /* on Android, mmap will fail despite RWX allocs are possible */
    ...
    debugp(stderr,"try fread(%p, 1, %llx, file); ", image, (unsigned long long)(UCell)imagesize);
...
#ifdef HAVE_MMAP
  debugp(stderr,"try unmmap(%p, $%llx); ", (void*)image, (unsigned long long)dictsize);
...
static int reserve_code_space(UCell size)
{
  ...
  if (code_area+code_area_size < code_here+size) {
    ...
    debugp(stderr, "Don't reserve code size %llu, code_area=%p, code_ares_size=%lld, code_here=%p\n", (unsigned long long)size, code_area, (long long)code_area_size, code_here);
  }
  return 0;
}
...
/* returns the address of the image proper (after the preamble) */
{
  ...
  for(i=0; i<0xFE; ) {
    ...
    fseek(imagefile, -(long)sizeof(SectionHeader), SEEK_CUR);
    debugp(stderr, "section base=%p, dp=%p, size=%llx\n", section.base, section.dp, (unsigned long long)(UCell)section.size);
  ...
  if (header.checksum==0)
    ...
    fprintf(stderr,"%s: Checksum of image ($%llx) does not match the executable ($%llx)\n",
	    progname, (unsigned long long)header.checksum, (unsigned long long)check_sum);
    return NULL;
  }
#ifdef DOUBLY_INDIRECT
...
void gforth_printmetrics()
{
  if (print_metrics) {
    long i;
    fprintf(stderr, "%8llu native code bytes generated (including deleted code) without padding\n", (unsigned long long)(generated_bytes - generated_nop_bytes));
    fprintf(stderr, "%8llu native code bytes generated (including deleted code)\n", (unsigned long long)generated_bytes);
    ...
    if (ip_update_metrics==NULL)
      ...
      for (i=0; i<nip_update_metrics; i++)
        if (ip_update_metrics[i] > 0)
          fprintf(stderr,"%5lld ip update by %2lld cells\n",
                  (long long)ip_update_metrics[i], (long long)(i+ip_update_metrics_base));
  }
  if (tpa_trace) {
    fprintf(stderr, "%ld %ld lb_states\n", lb_labeler_steps, lb_newstate_new);
    fprintf(stderr, "%ld %ld lb_table_entries\n", lb_labeler_steps, lb_labeler_dynprog);
...
void gforth_free_stacks(user_area * t)
{
  ...
  size += 2*SIGSTKSZ;
#endif
  debugp(stderr,"try munmap(%p, %llx); ", t, (unsigned long long)(UCell)size);
  r=munmap(t, size);
  if(r)
    fprintf(stderr,"munmap(%p, %llx) failed: %s\n", t, (unsigned long long)(UCell)size, strerror(errno));
  else
...
Cell gforth_main(int argc, char **argv, char **env)
{
  ...
  debugp(stderr, "Start returned %lld\n", (long long)retvalue);
