/* Gforth support functions
  ...
  Copyright (C) 1995,1996,1997,1998,2000,2003,2004,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025 Free Software Foundation, Inc.
...
static struct Cellquad gforth_read_console_line(Char *c_addr, UCell u1, FILE *wfileid)
{
  struct Cellquad r;
  HANDLE handle;
  ...
  if (!gforth_console_stream(wfileid, &handle)) {
    errno = ENOTTY;
    r.n4 = FILEIO(errno);
    return r;
  }
  ...
  line_mode = original_mode | ENABLE_LINE_INPUT | ENABLE_ECHO_INPUT | ENABLE_PROCESSED_INPUT;
  SetConsoleMode(handle, line_mode);

  request = (DWORD)((u1 > 0xffffu - 2u) ? 0xffffu : (u1 + 2u));
  if (request < 2)
    request = 2;

  buffer = (char *)malloc((size_t)request + 1);
  if (buffer == NULL) {
  ...
  if (!ReadConsoleA(handle, buffer, request, &chars_read, NULL)) {
    free(buffer);
    SetConsoleMode(handle, original_mode);
    errno = EINVAL;
    r.n4 = FILEIO(errno);
  ...
  for (u3 = 0; u3 < chars_read; u3++) {
    char c = buffer[u3];

    if (c == '\r') {
      saw_newline = 1;
      if (u3 + 1 < chars_read && buffer[u3 + 1] == '\n')
        u3++;
      break;
    }
    if (c == '\n') {
...
#ifdef _WIN32
  if (gforth_console_stream(wfileid, NULL))
    return gforth_read_console_line(c_addr, u1, wfileid);
#endif
