...
\ Copyright (C) 1995,1996,1997,1998,2000,2003,2006,2007,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025 Free Software Foundation, Inc.
...
: (type) ( c-addr u -- )
    dup out +!
    outfile-id dup >r write-file drop \ !! use ?DUP-IF THROW ENDIF instead of DROP ?
    r@ isatty IF  r@ flush-file drop  THEN
    rdrop
;

: (emit) ( c -- )
    1 out +!
    outfile-id dup >r emit-file drop \ !! use ?DUP-IF THROW ENDIF instead of DROP ?
    r@ isatty IF  r@ flush-file drop  THEN
    rdrop
;
...
: (err-type) ( c-addr u -- )
    dup out +!
    debug-fid dup >r write-file drop \ !! use ?DUP-IF THROW ENDIF instead of DROP ?
    r@ isatty IF  r@ flush-file drop  THEN
    rdrop
;
...
: (err-emit) ( c -- )
    1 out +!
    debug-fid dup >r emit-file drop \ !! use ?DUP-IF THROW ENDIF instead of DROP ?
    r@ isatty IF  r@ flush-file drop  THEN
    rdrop
;
